...
class HijackedFuncOfAclrtLaunchKernelV2Test : public ContextMockHelper {
};
...
TEST_F(HijackedFuncOfAclrtLaunchKernelV2Test, mock_valid_hijacked_input_then_test_call_expect_ok)
{
    ...
    uint32_t aaa = 10;
    ASSERT_EQ(inst.Call(funcHandle_, 3, &aaa, 4, &cfg, stream), ACL_SUCCESS);
}
...
TEST_F(HijackedFuncOfAclrtLaunchKernelV2Test, call_function_msprof_simulator_init)
{
    ...
    aclrtLaunchKernelCfg cfg = {nullptr, 0};
    inst.Pre(nullptr, 3, nullptr, 0, &cfg, nullptr);
...
TEST_F(HijackedFuncOfAclrtLaunchKernelV2Test, prof_gen_bbfile_and_dbifile_fail)
{
    ...
    uint32_t aaa = 10;
    inst.profObj_ = std::make_shared<ProfDataCollect>(nullptr);
    inst.Pre(funcHandle_, 3, &aaa, 4, &cfg, stream);
    EXPECT_TRUE(inst.Post(ACL_SUCCESS) == ACL_SUCCESS);
    EXPECT_TRUE(inst.profObj_ != nullptr);
    GlobalMockObject::verify();
}
