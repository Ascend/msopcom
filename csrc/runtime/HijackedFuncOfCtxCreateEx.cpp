...
rtError_t HijackedFuncOfCtxCreateEx::Call(void **ctx, uint32_t flags, int32_t devId)
{
    Pre(ctx, flags, devId);
    if (this->originfunc_ == nullptr) {
        ERROR_LOG("HijackedFuncOfCtxCreateEx originfunc is nullptr.");
        return RT_ERROR_RESERVED;
    }
    if (IsOpProf() && ProfConfig::Instance().IsSimulator()) {
        devId = 0;
    }
    rtError_t ret = this->originfunc_(ctx, flags, devId);
