...
void HijackedFuncOfMapMem::Pre(void* devPtr, size_t size, size_t offset, rtDrvMemHandle_t* handle, uint64_t flags)
{
    this->devPtr_ = devPtr;
    this->size_ = size;
}

rtError_t HijackedFuncOfMapMem::Post(rtError_t ret)
{
    if (IsSanitizer()) {
