...
HijackedFuncOfAclrtMallocForTaskSchedulerImpl::HijackedFuncOfAclrtMallocForTaskSchedulerImpl()
    : HijackedFuncType(AclRuntimeLibName(), "aclrtMallocForTaskSchedulerImpl"), devPtr_{nullptr}, size_{} {}

void HijackedFuncOfAclrtMallocForTaskSchedulerImpl::Pre(
...
aclError HijackedFuncOfAclrtMallocForTaskSchedulerImpl::Call(
    void **devPtr, size_t size, aclrtMemMallocPolicy policy, aclrtMallocConfig *cfg) {
    size_t actualSize = MemoryGuard::Instance().GetTotalSize(size);
    this->actualSize_ = actualSize;
    Pre(devPtr, size, policy, cfg);
    if (originfunc_) {
        aclError ret = originfunc_(devPtr, actualSize, policy, cfg);
        return Post(ret);
    }
    ERROR_LOG("HijackedFuncOfAclrtMallocForTaskScheduler originfunc is nullptr.");

    return EmptyFunc();
}

aclError HijackedFuncOfAclrtMallocForTaskSchedulerImpl::Post(aclError ret) {
    if (IsSanitizer()) {
